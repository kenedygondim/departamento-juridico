...
void adicionarAdvogado()
{
	...
	arquivoAdvogado = fopen("C:\\Users\\kened\\OneDrive\\Documentos\\C#\\Departamento Jurídico\\data\\ADVOGADOS.DAT", "a+b");
...
{
	int count = 0;
	arquivoAdvogado = fopen("C:\\Users\\kened\\OneDrive\\Documentos\\C#\\Departamento Jurídico\\data\\ADVOGADOS.DAT", "r+b");
	if (arquivoAdvogado == NULL)
	{
		printf("Erro ao abrir ADVOGADOS.DAT\n");
		system("pause");
	...
	arquivoAdvogado = fopen("C:\\Users\\kened\\OneDrive\\Documentos\\C#\\Departamento Jurídico\\data\\ADVOGADOS.DAT", "w+b"); //abriu o arquivo para sobreescrita
...
Advogado verAdvogadoEspecifico(int oab) {
	...
	if (!encontrado) {
		printf("\nNenhum advogado encontrado com a OAB: %d\n", oab);
	}
