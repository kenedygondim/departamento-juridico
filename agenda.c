...
void adicionarObservacaoAtendimento()
{
    ...
    while (fread(&agenda, sizeof(Agenda), 1, arquivoAgenda))
    {
        if (strcmp(agenda.numProcesso, numProcesso) == 0)
        {
            ...
            // Concatenar as novas observações às existentes
            if (strlen(novasObservacoes) > 0)
            {
                if (strlen(agenda.observacoes) > 0)
                {
                    strncat(agenda.observacoes, " ", sizeof(agenda.observacoes) - strlen(agenda.observacoes) - 1);
                }
                strncat(agenda.observacoes, novasObservacoes, sizeof(agenda.observacoes) - strlen(agenda.observacoes) - 1);
            }
